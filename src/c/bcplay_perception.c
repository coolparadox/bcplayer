...
int bc_perceive(struct bc_perception* result) {
    ...
    if (!display) { log_err("%s: cannot open display", __func__); return 1; }
    Window root = DefaultRootWindow(display);
    XWindowAttributes attr = { 0 };
    XGetWindowAttributes(display, root, &attr);
    if (attr.width != BC_KIOSK_WIDTH) { log_err("%s: unexpected screen width %u", __func__, attr.width); return 1; }
    if (attr.height != BC_KIOSK_HEIGHT) { log_err("%s: unexpected screen height %u", __func__, attr.height); return 1; }
    XImage* img = XGetImage(display, root, 0, 0, BC_KIOSK_WIDTH, BC_KIOSK_HEIGHT, AllPlanes, ZPixmap);
    if (!img) { log_err("cannot screenshot"); return 1; }
    ...
    for (unsigned int row = 0; row < BC_KIOSK_HEIGHT; ++row) for (unsigned int col = 0; col < BC_KIOSK_WIDTH; ++col) {
        ...
        if (rgb) { log_err("%s: unsupported pixel format", __func__); return 1; }
    }
    XDestroyImage(img);
